...
interface CreateGroupStepProps {
 ...
 const CreateGroupStep: React.FC<CreateGroupStepProps> = ({ onSubmit, isSubmitting }) => {
   ...
   useEffect(() => {
     const fetchInitialData = async () => {
       try {
         const [skillsData, semestersData, subjectsData, fieldssData] =
           await Promise.all([
             getAllSkills(),
   ...
   useEffect(() => {
     const subscription = form.watch(() => {
       ...
       const hasValidPositions = formValues.groupPositions.length > 0 && 
         formValues.groupPositions.every(position => 
           position.name?.trim() !== "" && 
           position.count > 0 && 
